...
После установки перейти в раздел "Settings" во вкладке "Kubernetes", включить опцию "Enable Kubernetes", выбрав метод "Kubeadm". Дождаться пока кластер инициализируется
...
docker pull aggrik/gitbucket:latest
...
```
#Применяем обязательно перед запуском других контейнеров
kubectl apply -f ./ML/env/

#Постгрес идет в связке с mlflow - поднимать нужно оба
kubectl apply -f ./ML/postgres-mlflow/
kubectl apply -f ./ML/mlflow/
#Постгрес идет в связке с superset - поднимать нужно оба
kubectl apply -f ./ML/postgres-superset/
kubectl apply -f ./ML/superset/
#Постгрес идет в связке с airflow - поднимать нужно оба
kubectl apply -f ./ML/postgres-airflow/
kubectl apply -f ./ML/airflow/
#Поднимать можно только те, которые необходимы
kubectl apply -f ./ML/clickhouse/
kubectl apply -f ./ML/kafka/
kubectl apply -f ./ML/zookeeper/
kubectl apply -f ./ML/gitbucket/
kubectl apply -f ./ML/greenplum/
kubectl apply -f ./ML/jupyter/
kubectl apply -f ./ML/minio/
kubectl apply -f ./ML/tarantool/
```
...
```
http://localhost:32051/gitbucket/
login - admin
passowrd - admin
```
